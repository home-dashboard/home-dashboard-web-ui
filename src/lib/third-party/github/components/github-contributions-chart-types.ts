...
/**
 * 自适应贡献图表的列数.
 * @param tableElementRect 贡献图表的表格元素的尺寸
 * @param measuredElementRect 用于测量的单元格元素的尺寸
 * @param cell 单元格的尺寸
 * @param maxColumn 最大列数
 */
export function adaptiveColumn(
  tableElementRect: DOMRectReadOnly,
  measuredElementRect: DOMRectReadOnly,
  cell: DayCell,
  maxColumn: number
): number {
  const { width: tableWidth } = tableElementRect;
  const { width: labelWidth } = measuredElementRect;
...
export function calculateRenderedStartColumnIndex(
  ...
  // 处理约束规则 3
  // index 是某个月中的第一列时, 直接返回. 否则渲染下一个月的第一列.
